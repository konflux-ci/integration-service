...
var _ = Describe("Utils", func() {
	...
	When("pipeline type labels are evaluated", func() {
		...
		DescribeTable("should detect new-model build PipelineRuns",
			...
			Entry("new type annotation is not a new-model label", &tektonv1.PipelineRun{ObjectMeta: v1.ObjectMeta{
				Labels:      map[string]string{keys.PipelineType().Old: "build"},
				Annotations: map[string]string{keys.PipelineType().New: "build"},
			}}, false),
