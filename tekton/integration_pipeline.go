...
func NewIntegrationPipelineRun(client client.Client, ctx context.Context, loader loader.ObjectLoader, prefix, namespace string, integrationTestScenario *v1beta2.IntegrationTestScenario, snapshot *applicationapiv1alpha1.Snapshot) (*IntegrationPipelineRun, error) {
	...
	if ShouldUpdateIntegrationPipelineGitResolver(integrationTestScenario, snapshot) {
		log.Info("use the integration test pipeline/pipelinerun from the code in the pr of snapshot", "integrationTestScneario.Name", integrationTestScenario.Name)
