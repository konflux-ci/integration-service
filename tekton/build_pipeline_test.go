...
var _ = Describe("build pipeline", Ordered, func() {
	...
	Context("When a build pipelineRun exists", func() {
		...
		It("treats commit-comment GitOps PipelineRuns as push even with a pull-request label", func() {
		...
		It("does not treat PR-comment GitOps PipelineRuns as push", func() {
			plr := buildPipelineRun.DeepCopy()
			plr.Labels[tektonconsts.PipelineAsCodeEventTypeLabel] = "test-comment"
			plr.Labels[tektonconsts.PipelineAsCodePullRequestLabel] = "42"
		...
		It("treats tag commit-comment GitOps as push", func() {
			plr := buildPipelineRun.DeepCopy()
