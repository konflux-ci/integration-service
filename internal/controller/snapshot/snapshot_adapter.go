...
func (a *Adapter) EnsureGlobalCandidateImageUpdated() (controller.OperationResult, error) {
	...
	if err != nil {
		...
		addedToGlobalCandidateListStatus = gitops.AddedToGlobalCandidateListStatus{
			Result:          false,
			Reason:          fmt.Sprintf("Failed to set Global Candidate List according to override snapshot %s/%s due to error %s", a.snapshot.Namespace, a.snapshot.Name, loaderError.Error()),
