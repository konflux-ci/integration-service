...
var _ = Describe("Snapshot Adapter", Ordered, func() {
	var (
		...
		hasOldGroupSnapshotPR                     *applicationapiv1alpha1.Snapshot
		hasNewGroupSnapshotPR                     *applicationapiv1alpha1.Snapshot
	...
	const (
		...
		hasNewGroupSnapshotPRName = "hasnewgroupsnapshotpr-sample"
		prGroup                   = "feature1"
	...
	BeforeEach(func() {
		...
		hasOldGroupSnapshotPR = &applicationapiv1alpha1.Snapshot{
			ObjectMeta: metav1.ObjectMeta{
				...
				Labels: map[string]string{
					...
					gitops.PRGroupAnnotation:               "a-team",
				},
				Annotations: map[string]string{
		...
		Expect(k8sClient.Create(ctx, hasOldGroupSnapshotPR)).Should(Succeed())

		hasNewGroupSnapshotPR = &applicationapiv1alpha1.Snapshot{
			ObjectMeta: metav1.ObjectMeta{
				Name:      hasNewGroupSnapshotPRName,
				Namespace: "default",
				Labels: map[string]string{
					gitops.SnapshotTypeLabel:               gitops.SnapshotGroupType,
					gitops.SnapshotComponentLabel:          "component-sample",
		...
		integrationPipelineRunOld = &tektonv1.PipelineRun{
			ObjectMeta: metav1.ObjectMeta{
				...
				Labels: map[string]string{
					...
					"appstudio.openshift.io/snapshot":          hasOldGroupSnapshotPR.Name,
					"pipelinesascode.tekton.dev/event-type":    "pull_request",
					"build.appstudio.redhat.com/target_branch": "main",
					"test.appstudio.openshift.io/pr-group-sha": prGroupSha,
				},
	...
	AfterEach(func() {
		...
		err = k8sClient.Delete(ctx, hasOldGroupSnapshotPR)
		Expect(err == nil || errors.IsNotFound(err)).To(BeTrue())
		err = k8sClient.Delete(ctx, hasNewGroupSnapshotPR)
		Expect(err == nil || errors.IsNotFound(err)).To(BeTrue())
	...
	When("old integration pipelinerun gets cancelled because newer pipelinerun gets triggered", func() {
		...
		It("ensures that old group snapshot together with integration pipelinerun has been canceled", func() {
			...
			adapter = NewAdapter(ctx, hasOldGroupSnapshotPR, hasApp, log, loader.NewMockLoader(), k8sClient)
			adapter.context = toolkit.GetMockedContext(ctx, []toolkit.MockData{
				{
					ContextKey: loader.ApplicationContextKey,
					Resource:   hasApp,
				...
				{
					ContextKey: loader.SnapshotContextKey,
					Resource:   hasOldGroupSnapshotPR,
				},
				{
					ContextKey: loader.SnapshotComponentsContextKey,
					Resource:   []applicationapiv1alpha1.Component{*hasComp},
			...
			Expect(k8sClient.Create(adapter.context, hasNewGroupSnapshotPR)).Should(Succeed())
			Eventually(func() error {
				err := k8sClient.Get(adapter.context, types.NamespacedName{
					Name:      hasNewGroupSnapshotPR.Name,
					Namespace: "default",
				}, hasNewGroupSnapshotPR)
				return err
			}, time.Second*10).ShouldNot(HaveOccurred())
			result, err := adapter.EnsureIntegrationPipelineRunsExist()
			Expect(result.CancelRequest).To(BeFalse())
			Expect(result.RequeueRequest).To(BeFalse())
			foundStatusCondition := meta.FindStatusCondition(hasOldGroupSnapshotPR.Status.Conditions, gitops.AppStudioIntegrationStatusCondition)
			Expect(foundStatusCondition.Reason).To(Equal(gitops.AppStudioIntegrationStatusCanceled))
			Expect(err).ToNot(HaveOccurred())
			statuses, err := gitops.NewSnapshotIntegrationTestStatusesFromSnapshot(hasOldGroupSnapshotPR)
			Expect(err).ToNot(HaveOccurred())
			detail, ok := statuses.GetScenarioStatus(integrationTestScenario.Name)
			Expect(ok).To(BeTrue())
			Expect(detail.Status).To(Equal(intgteststat.IntegrationTestStatusInProgress))
			...
			pipelineRuns, err := loader.GetAllPipelineRunsForSnapshotAndScenario(ctx, k8sClient, hasOldGroupSnapshotPR, integrationTestScenario)
		...
		It("ensures that old component snapshot together with integration pipelinerun has been canceled", func() {
			...
			Expect(foundStatusCondition.Reason).To(Equal(gitops.AppStudioIntegrationStatusCanceled))
			Expect(err).ToNot(HaveOccurred())
			statuses, err := gitops.NewSnapshotIntegrationTestStatusesFromSnapshot(hasOldCompSnapshotPR)
			Expect(err).ToNot(HaveOccurred())
			detail, ok := statuses.GetScenarioStatus(integrationTestScenario.Name)
			Expect(ok).To(BeTrue())
			Expect(detail.Status).To(Equal(intgteststat.IntegrationTestStatusInProgress))
