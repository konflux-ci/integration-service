...
var _ = Describe("PipelineController", func() {
	...
	When("a new-model build PipelineRun is reconciled", func() {
		...
		It("should remove IntegrationPipelineRunFinalizer when the PipelineRun is deleting", func() {
			...
			controllerutil.AddFinalizer(newModelPipelineRun, helpers.NudgePipelineRunFinalizer)
		...
		It("should remove NudgePipelineRunFinalizer when the PipelineRun is deleting", func() {
			controllerutil.AddFinalizer(newModelPipelineRun, helpers.NudgePipelineRunFinalizer)
			Expect(k8sClient.Update(ctx, newModelPipelineRun)).To(Succeed())

			Expect(k8sClient.Delete(ctx, newModelPipelineRun)).To(Succeed())
			// The new-model label keys gate this PipelineRun from the normal build
			// reconciliation path. Verify it is terminating with the nudge finalizer
			// still present before exercising the gate's cleanup path.
			Eventually(func(g Gomega) {
				g.Expect(k8sClient.Get(ctx, reqNewModel.NamespacedName, newModelPipelineRun)).To(Succeed())
				g.Expect(newModelPipelineRun.GetDeletionTimestamp()).NotTo(BeNil())
				g.Expect(controllerutil.ContainsFinalizer(newModelPipelineRun, helpers.NudgePipelineRunFinalizer)).To(BeTrue())
			}, time.Second*10).Should(Succeed())
