	...
	Context("Testing override snapshot GCL update", func() {
		...
		When("The GCL is updated for override snapshot", func() {
			BeforeEach(func() {
				...
				// Expect(UpdateGCLForOverrideSnapshot(ctx, k8sClient, hasCompGroup, overrideSnapshot, log)).To(Succeed())
				err := UpdateGCLForOverrideSnapshot(ctx, k8sClient, hasCompGroup, overrideSnapshot, log)
				Expect(err).NotTo(HaveOccurred())
				time.Sleep(10)
