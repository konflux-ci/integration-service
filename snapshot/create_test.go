...
var _ = Describe("Snapshot creation functions", Ordered, func() {
	...
	Context("Testing getAllNestedSnapshotComponents()", func() {
		It("returns GCL components from the componentGroup itself when there are no nested groups", func() {
			snapshotComponents, invalidComponents, err := getAllNestedSnapshotComponents(hasCompGroup, []string{}, 0, loader.NewMockLoader(), mockCtx, k8sClient, logger)
			Expect(err).ToNot(HaveOccurred())
			Expect(snapshotComponents).To(HaveLen(1))
			Expect(snapshotComponents).To(HaveKey(helpers.GetComponentVersionString(componentName, "v1")))
		...
		It("returns an error when a cycle is detected in nested componentGroups", func() {
			_, _, err := getAllNestedSnapshotComponents(hasCompGroup, []string{hasCompGroup.Name}, 0, loader.NewMockLoader(), mockCtx, k8sClient, logger)
			Expect(err).To(HaveOccurred())
		...
		It("returns an error when the loader fails to get nested component groups", func() {
			...
			_, _, err := getAllNestedSnapshotComponents(hasCompGroup, []string{}, 0, errLoader, ctx, k8sClient, logger)
			Expect(err).To(HaveOccurred())
		...
		It("merges components from a nested componentGroup with the parent's own GCL", func() {
			...
			snapshotComponents, _, err := getAllNestedSnapshotComponents(hasCompGroup, []string{}, 0, nestedLoader, ctx, k8sClient, logger)
			Expect(err).ToNot(HaveOccurred())
			Expect(snapshotComponents).To(HaveKey(helpers.GetComponentVersionString(componentName, "v1")))
