...
// BatchDefaults defines namespace-wide timing and failure behavior for batched nudges:
// when several source builds nudge the same target close together, integration-service
// can coalesce them into one target build after debounceTimeout or at maxWaitTime.
// This field is reserved; no controller yet applies these defaults.
// The validating webhook emits an admission warning when batchDefaults is set.
//
// kubebuilder CEL cannot reference Go constants; the duration('30m') and duration('4h')
// literals in the XValidation markers must stay aligned with DefaultBatchDebounceTimeout
// and DefaultBatchMaxWaitTime. TestBatchDefaultsCELLiteralsMatchRuntimeDefaults enforces
// that parity on nudgeconfig_types.go and the generated CRD (run via make test in PR CI).
...
// Unset fields inherit from spec.batchDefaults, then runtime defaults. Reserved until batching is implemented.
//
// +kubebuilder:validation:XValidation:rule="!has(self.debounceTimeout) || !has(self.maxWaitTime) || duration(self.maxWaitTime) > duration(self.debounceTimeout)",message="maxWaitTime must exceed debounceTimeout"
// +kubebuilder:validation:XValidation:rule="!has(self.debounceTimeout) || has(self.maxWaitTime) || duration(self.debounceTimeout) < duration('4h')",message="debounceTimeout must be less than the 4h runtime default for maxWaitTime when maxWaitTime is omitted"
// +kubebuilder:validation:XValidation:rule="!has(self.maxWaitTime) || has(self.debounceTimeout) || duration(self.maxWaitTime) > duration('30m')",message="maxWaitTime must exceed the 30m runtime default for debounceTimeout when debounceTimeout is omitted"
type BatchPolicy struct {
	// DebounceTimeout is how long to wait after the last incoming build before firing the batch.
	// Range: 1m–24h. Inherits from batchDefaults or 30m runtime default when unset.
	// +kubebuilder:validation:Type=string
	// +kubebuilder:validation:Format=duration
	// +kubebuilder:validation:XValidation:rule="duration(self) >= duration('1m') && duration(self) <= duration('24h')",message="debounceTimeout must be between 1m and 24h"
	// +optional
	...
	// MaxWaitTime is the hard deadline from the first build event, regardless of debounce resets.
	// Range: 1m–24h. Inherits from batchDefaults or 4h runtime default when unset.
	// +kubebuilder:validation:Type=string
	// +kubebuilder:validation:Format=duration
	// +kubebuilder:validation:XValidation:rule="duration(self) >= duration('1m') && duration(self) <= duration('24h')",message="maxWaitTime must be between 1m and 24h"
	// +optional
	...
	// FailurePolicy controls whether a failed member build blocks the batch or allows a partial fire.
	// Inherits from batchDefaults or Block runtime default when unset.
	// +optional
	FailurePolicy *FailurePolicyType `json:"failurePolicy,omitempty"`
}
...
type TargetConfig struct {
	...
	// BatchPolicy defines per-target batch timing and failure behavior overrides.
	// When omitted, the target is not batched. An empty object opts into batching with inherited defaults.
	// +optional
...
// +kubebuilder:validation:XValidation:rule="!has(self.targetConfig) || self.targetConfig.all(i, self.targetConfig.exists_one(j, i.target == j.target))",message="duplicate target not allowed in targetConfig"
type NudgeConfigSpec struct {
	// BatchDefaults sets default debounce, max wait, and failure policy for batched nudges
	// to shared targets in this namespace (see BatchDefaults). Reserved until batching is implemented.
	// +optional
...
const (
	// BatchPhaseAccumulating is the phase when the batch is collecting build events within the batch window.
	BatchPhaseAccumulating BatchPhase = "Accumulating"

	// BatchPhaseBlocked is the phase when the batch cannot proceed due to a dependency or gate condition.
	BatchPhaseBlocked BatchPhase = "Blocked"

	// BatchPhaseFiring is the phase when the batch is actively creating the nudge pipeline run.
	BatchPhaseFiring BatchPhase = "Firing"

	// BatchPhaseFailed is the phase when the batch encountered an unrecoverable error.
	BatchPhaseFailed BatchPhase = "Failed"

	// BatchPhaseCompleted is the phase when the batch successfully created the nudge pipeline run.
...
// FailedEntry defines a build event that could not be accumulated into the batch.
type FailedEntry struct {
	// From is the source component name whose build failed to be accumulated.
	// +kubebuilder:validation:MinLength=1
	// +kubebuilder:validation:MaxLength=63
	// +kubebuilder:validation:Pattern=`^[a-z0-9]([-a-z0-9]*[a-z0-9])?$`
	// +required
	...
	// BuildPipelineRun is the name of the build PipelineRun associated with the failure.
	// +kubebuilder:validation:MinLength=1
	// +required
	BuildPipelineRun string `json:"buildPipelineRun"`

	// Reason is a human-readable explanation of why this entry failed.
	// +kubebuilder:validation:MinLength=1
	// +required
	Reason string `json:"reason"`

	// CapturedAt is the timestamp when this failure was recorded.
	// +required
	CapturedAt metav1.Time `json:"capturedAt"`
}

// ActiveBatch defines the runtime state of a single nudge batch.
// All batch state is persisted here so it survives controller restarts.
type ActiveBatch struct {
	...
	// Nil when the batch is in Blocked phase. Semantics aligned with ADR 72 (landing in parallel).
	// +optional
	FireAt *metav1.Time `json:"fireAt,omitempty"`

	// HardDeadline is the absolute deadline by which an accumulating batch must fire,
	// regardless of debounce timer resets. If passed while blocked, the batch fires
	// immediately once unblocked rather than being abandoned.
	// +required
	...
	// Failed is the list of build events that could not be accumulated into this batch.
	...
	// RetryCount is the number of times this batch has been retried after failure.
	// Persisted to survive controller restarts.
	// +optional
...
type NudgeConfigStatus struct {
	...
	//   The validating webhook also warns on create/update when batchDefaults is present.
	// +optional
	...
	// ActiveBatches tracks the runtime state of all in-progress nudge batches.
	// Batch lifecycle (including removal of terminal entries) is managed by the controller.
	// +listType=map
	// +listMapKey=batchId
	// +kubebuilder:validation:MaxItems=360
	// +optional
