...
func TestActiveBatchDeepCopy(t *testing.T) {
	...
	// Mutate FireAt pointer and verify original is unchanged
	newFireAt := metav1.NewTime(now.Add(99 * time.Minute))
	copied.FireAt = &newFireAt

	if original.FireAt.Equal(&newFireAt) {
		t.Errorf("Original FireAt was modified after mutating copy")
	}
}
