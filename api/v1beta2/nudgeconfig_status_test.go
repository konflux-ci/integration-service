...
func TestActiveBatchDeepCopy(t *testing.T) {
	...
	// Mutate through FireAt pointer and verify original is unchanged
	originalFireAt := *original.FireAt
	*copied.FireAt = metav1.NewTime(now.Add(99 * time.Minute))

	if !original.FireAt.Equal(&originalFireAt) {
		t.Errorf("Original FireAt was modified after mutating through copy pointer")
	}
}

func TestActiveBatchOptionalFieldsOmitted(t *testing.T) {
	now := metav1.Now()
	batch := ActiveBatch{
		Target:       "component-x",
		BatchID:      "batch-min",
	...
	if batch.FireAt != nil {
		t.Errorf("Expected nil FireAt, got %v", batch.FireAt)
	}
	if batch.Accumulated != nil {
		t.Errorf("Expected nil Accumulated, got %v", batch.Accumulated)
	}
	if batch.Failed != nil {
		t.Errorf("Expected nil Failed, got %v", batch.Failed)
	}
	if batch.NudgePipelineRun != "" {
		t.Errorf("Expected empty NudgePipelineRun, got '%s'", batch.NudgePipelineRun)
	}
	...
	if batch.RetryCount != 0 {
		t.Errorf("Expected zero RetryCount, got %d", batch.RetryCount)
	}
}
