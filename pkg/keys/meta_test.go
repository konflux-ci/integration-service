...
var _ = Describe("read either", func() {
	...
	When("detecting new-model keys", func() {
		...
		It("should see a new-style annotation", func() {
			...
			Expect(keys.HasNewLabel(obj, pair)).To(BeFalse())
		})
	})
})

var _ = Describe("built-in key pairs", func() {
	It("returns copies that cannot mutate the package defaults", func() {
		pipelineType := keys.PipelineType()
		pipelineType.Old = "changed"
		pipelineType.New = "changed"
		buildComponent := keys.BuildComponent()
		buildComponent.Old = "changed"
		buildComponent.New = "changed"

		Expect(keys.PipelineType()).To(Equal(keys.Pair{
			Old: keys.PrefixPipelinesOld + "/type",
			New: keys.PrefixPipelinesNew + "/type",
		}))
		Expect(keys.BuildComponent()).To(Equal(keys.Pair{
			Old: keys.PrefixAppstudio + "/component",
			New: keys.PrefixBuildNew + "/component",
		}))
	})
})
