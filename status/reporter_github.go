...
func (cru *CheckRunStatusUpdater) Authenticate(ctx context.Context, object metav1.Object) error {
	...
	if err != nil {
		cru.logger.Error(err, "failed to get app credentials from Object",
...
func (cru *CheckRunStatusUpdater) createCheckRunAdapterForObject(report TestReport) (*github.CheckRunAdapter, error) {
	...
	if err != nil {
		cru.logger.Error(err, fmt.Sprintf("failed to generate conclusion for integrationTestScenario %s and object %s/%s", report.ScenarioName, object.GetNamespace(), object.GetName()))
		return nil, fmt.Errorf("unknown status %s for integrationTestScenario %s and object %s/%s", report.Status, report.ScenarioName, object.GetNamespace(), object.GetName())
	}

	title, err := generateCheckRunTitle(report.Status)
	if err != nil {
		cru.logger.Error(err, fmt.Sprintf("failed to generate title for integrationTestScenario %s and object %s/%s", report.ScenarioName, object.GetNamespace(), object.GetName()))
		return nil, fmt.Errorf("failed to generate title for integrationTestScenario %s and %s/%s", report.ScenarioName, object.GetNamespace(), object.GetName())
	}
...
func (cru *CheckRunStatusUpdater) UpdateStatus(ctx context.Context, report TestReport) error {
	...
	if existingCheckrun == nil {
		cru.logger.Info("creating checkrun for scenario test status of snapshot",
			"object.NameSpace", cru.object.GetNamespace(), "object.Name", cru.object.GetName(), "scenarioName", report.ScenarioName, "externalID", checkRunAdapter.ExternalID)
...
func (csu *CommitStatusUpdater) getAllCommitStatuses(ctx context.Context) ([]*ghapi.RepoStatus, error) {
	if len(csu.allCommitStatusesCache) == 0 {
		...
		if err != nil {
			csu.logger.Error(err, "failed to get all commitStatuses for object",
...
func (csu *CommitStatusUpdater) Authenticate(ctx context.Context, object metav1.Object) error {
	...
	if err != nil {
		csu.logger.Error(err, "failed to get token from object",
			"object.NameSpace", csu.object.GetNamespace(), "object.Name", csu.object.GetName())
		return err
	}
...
func (csu *CommitStatusUpdater) createCommitStatusAdapterForSnapshot(report TestReport) (*github.CommitStatusAdapter, error) {
	...
	if err != nil {
		csu.logger.Error(err, fmt.Sprintf("failed to generate commitStatus for integrationTestScenario %s and object %s/%s", report.ScenarioName, object.GetNamespace(), object.GetName()))
		return nil, fmt.Errorf("unknown status %s for integrationTestScenario %s and object %s/%s", report.Status, report.ScenarioName, object.GetNamespace(), object.GetName())
	}
...
func (csu *CommitStatusUpdater) UpdateStatus(ctx context.Context, report TestReport) error {
	...
	if csu.owner == sourceRepoOwner {
		...
		if !commitStatusExist {
			...
			if err != nil {
				csu.logger.Error(err, "failed to create commitStatus", "object.NameSpace", csu.object.GetNamespace(), "object.Name", csu.object.GetName(), "scenarioName", report.ScenarioName)
				return err
			}
		} else {
		...
		csu.logger.Info("Won't create/update commitStatus since there is access limitation for different source and target Repo Owner",
			"object.NameSpace", csu.object.GetNamespace(), "object.Name", csu.object.GetName(), "sourceRepoOwner", sourceRepoOwner, "targetRepoOwner", csu.owner)
	}
	...
	if report.Status != intgteststat.IntegrationTestStatusPending && report.Status != intgteststat.IntegrationTestStatusInProgress && isPullRequest {
		...
		if err != nil {
			csu.logger.Error(err, "failed to update comment", "object.NameSpace", csu.object.GetNamespace(), "object.Name", csu.object.GetName(), "scenarioName", report.ScenarioName)
			return err
		}
	}
...
func (r *GitHubReporter) Initialize(ctx context.Context, object metav1.Object) error {
	...
	if !found {
		...
		r.logger.Error(unRecoverableError, "object.NameSpace", object.GetNamespace(), "object.Name", object.GetName())
		return unRecoverableError
	}
	...
	if err := r.updater.Authenticate(ctx, object); err != nil {
		r.logger.Error(err, fmt.Sprintf("failed to authenticate for object %s/%s", object.GetNamespace(), object.GetName()))
		return err
	}
	return nil
}
...
func (r *GitHubReporter) ReportStatus(ctx context.Context, report TestReport) error {
	if r.updater == nil {
		r.logger.Error(nil, fmt.Sprintf("reporter is not initialized for snapshot %s", report.ObjectName))
		return fmt.Errorf("reporter is not initialized")
	}

	if err := r.updater.UpdateStatus(ctx, report); err != nil {
		r.logger.Error(err, fmt.Sprintf("failed to update status for snapshot %s", report.ObjectName))
		return err
	}
	return nil
}
